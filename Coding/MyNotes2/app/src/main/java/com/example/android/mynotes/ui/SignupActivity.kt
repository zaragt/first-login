    ...
    private fun setUpPasswordTextWatcher() {
        binding.password.editText?.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(editable: Editable?) {
                ...
                if (password.isNotEmpty()) {
                    binding.errorTextCPassword.visibility = View.INVISIBLE
                }
            }
        })
        ...
        binding.password.editText?.setOnFocusChangeListener { _, hasFocus ->
            if (!hasFocus) {
                val password = binding.password.editText?.text.toString().trim()
                if (password.isEmpty()) {
                    binding.errorTextCPassword.visibility = View.VISIBLE
                    binding.errorTextCPassword.text = "Password is required!"
                } else {
                    val missingCriteria = checkPasswordStrength(password)
                    if (missingCriteria.isNotEmpty()) {
                        binding.errorTextCPassword.visibility = View.VISIBLE
                        binding.errorTextCPassword.text = "Password must include: $missingCriteria"
                    } else {
                        binding.errorTextCPassword.visibility = View.INVISIBLE
                    }
                }
